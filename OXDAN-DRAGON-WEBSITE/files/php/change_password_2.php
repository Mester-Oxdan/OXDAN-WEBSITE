...
if (!isset($_SESSION['verified_password_reset'])) {
    echo json_encode(['status' => 'error', 'message' => 'Reset session expired or invalid.']);
    exit;
}
...
if (empty($resetData['csrf_token']) || empty($data['csrf_token']) || 
    !hash_equals($resetData['csrf_token'], $data['csrf_token'])) {
...
try {
    ...
    $subject = 'Oxdan Email ⚠️ Password Reset Notification.';
    $plaintext_body = "Hi, the password for your account {$user['username']} has been reset.
    IP: $suspicious_ip
    ...
    $html_body = "
    <h1>Oxdan Email ⚠️ Password Reset Notification.</h1>
    <p>Hi, the password for your account <b>{$user['username']}</b> has been successfully reset.</p>
    <ul>
